...
def solve_sys_ode(dF: npt.NDArray, h: float, rk4: npt.NDArray, N: int) -> npt.NDArray:
    ...
    return tSpan, Energy, Nt, fe, p


def main():
    t, Energy, Nt, fe, p = solve_sys_ode(dF, dt, rk4, N)
    T, E = np.meshgrid(t, Energy)
    T, P = np.meshgrid(t, Nt)
    fe = np.array(fe)
    p = np.array(p)
