...
t_max = 1000                       # femtosecond  500
t_0 = -3 * delta_t                 # femtosecond
e_max = 300                        # meV
T20 = 210                          # femtosecond
dt = 2                             # femtosecond
khi_o = 0.1  # 0.001, 0.1, 0.5    # Tham số cường độ xung (0.1 - 2)
...
for n in range(N):
    for n1 in range(N):
        if n1 != n:
            g[n, n1] = (1 / sqrt((n + 1) * delta_e)) * log(abs((sqrt_n[n] + sqrt_n[n1]) / (sqrt_n[n] - sqrt_n[n1]))) # Hệ số tương tác giữa mức năng lượng n với n1?
...
def FT(t, Pt):
    ...
    write_FT('Absorption Spectra Coulomb khi0 = 0.1 T(t) N=1000.txt', H, alpha ,np.real(P), np.imag(P), np.real(E), np.imag(E))
...
def write_Nt_Pt(filename, steps, Nt, Pt, PT, T2):
    with open(filename, 'w') as f:
        f.write(f"#{'t':^20}{'Nt':^20}{'Pt':^20}{'|Pt|':^20}{'T2':^20}\n")
        for i, t in enumerate(steps):
                f.write(f"{t:^20.5f}{Nt[i]:^20.5e}{Pt[i]:^20.5e}{PT[i]:^20.5e}{T2[i]:^20.5e}\n")
...
def main():
    ...
    for i, t in enumerate(tqdm(steps, desc="Processing", unit="steps")):
        Y, Nt[i], T2_values[i], Pt[i], PT[i] = RK4(F, t, Y, T20)
    ...
    write_result('Data Coulomb khi0 = 0.5 T(t).txt', steps, epsilon, Y0_real, Y0_imag, Y1_abs)
    write_Nt_Pt('Nt Pt Coulomb khi0 = 0.001 T=210.txt',steps, Nt, np.abs(Pt), PT, T2_values)

    end_time = time.time()
    print(f"Thời gian chạy: {end_time - start_time:.2f} giây!!!")
    #plot(steps, epsilon, Y0_real, Y1_abs, Nt, np.abs(Pt))
