    ...
    if beta <= 1:
        for j in tqdm(range(0, t)):
            for i in range(1, x):
                U[i][j + 1] = (
                    1
                    / (1 / (c**2 * dt**2) + frictionConstant / dt)
                    * (
                        (U[i + 1][j] - 2 * U[i][j] + U[i - 1][j]) / dL**2
                        - (U[i][j - 1] - 2 * U[i][j]) / (c**2 * dt**2)
                        + frictionConstant / dt * U[i][j - 1]
                    )
                )
...
splot "{file}" using 1:2:3 with lines tit "Wave",\
"{fileFric}" u 1:2:3 w l 
...
def main():
    ...
    fileFric = "HyperbolicEquationDataFric.txt"
    # writeLog(L, t, file)
    writeLog(L, t, fileFric)
    gnuPlot(file, fileFric)
